...
class UniXcoder(nn.Module):
    def __init__(self, model_name, special_tokens_path=None):
        """
            Build UniXcoder.
        ...
        # Add special tokens if path is provided
        if special_tokens_path and os.path.exists(special_tokens_path):
            print(f"Adding Special tokens from {special_tokens_path}")
            try:
                with open(special_tokens_path, "r", encoding="utf8") as special_tokens_file:
                    file_content = special_tokens_file.readlines()
                    if file_content:
                        special_tokens = file_content[0].split(" ")
                        special_tokens_dict = {'additional_special_tokens': special_tokens}
                        num_added_toks = self.tokenizer.add_special_tokens(special_tokens_dict)
                        self.model.resize_token_embeddings(len(tokenizer))
                        special_tokens_file.close()
            except Exception as e:
                print(f"Error loading special tokens: {e}")
            print("Done Adding Special tokens")
    ...
    def save_model_with_special_vocab(self, output_dir):
        
        """
            Save the model with its enhanced vocabulary for further easier usage
    ...
    # Save the tokenizer with its enhanced vocabulary
        self.tokenizer.save_pretrained(output_dir)
    
    # Save the config
        self.config.save_pretrained(output_dir)
    
    # Save additional metadata about special tokens
        special_tokens_info = {
        "vocab_size": len(self.tokenizer),
        "num_special_tokens": len(self.tokenizer.all_special_tokens),
        "special_tokens": self.tokenizer.all_special_tokens,
        }
    
        with open(os.path.join(output_dir, "special_tokens_info.json"), "w") as f:
            json.dump(special_tokens_info, f, indent=2)
        
        print(f"Model saved with vocabulary size: {len(self.tokenizer)}")
